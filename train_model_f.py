#!/usr/bin/env python3
"""
Training Script for Model F: CLAHE Enhanced Images with Partial Mask Overlay

Model F Configuration:
- CLAHE enhancement using optimized parameters from JSON
- Mask overlay at 0.5 opacity (partial overlay)
...
def main():
    parser = argparse.ArgumentParser(description='Train Model F: CLAHE + Partial Mask Overlay with Integrated Preprocessing')
    ...
    train_dataset = CLAHEDatasetWithMaskGeneration(
        ...
        temp_dir="temp_preprocessing_train_f"
    )
    ...
    val_dataset = CLAHEDatasetWithMaskGeneration(
        ...
        mask_opacity=0.5,  # Model F uses half opacity
        img_size=args.img_size,
        temp_dir="temp_preprocessing_val_f"
    )
    ...
    print(f"Model: {args.backbone} backbone with CLAHE + Partial Mask integration")
